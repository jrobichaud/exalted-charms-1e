# prerequisite-link-conversion Specification

## Purpose

Defines the behavior of the `scripts/link_prerequisites.py` post-processor that scans the Obsidian-flavored markdown corpus under `docs/markdown/`, finds every `Prerequisite Charms:` line in charm prose, and rewrites referenced charm names as Obsidian wikilinks. Includes the heading index, resolution cascade, wikilink formatting, idempotency guarantees, and report file emission required for the conversion to be safe and reproducible.
